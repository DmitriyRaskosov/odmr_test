    ...
    while (running) {
        ...
        if (ret > 0 && (pfd.revents & POLLIN)) {
            ...
            while (true) {
                ...
                if(hdr->tp_snaplen == 1066){
                    unsigned char num = packet[0x2a];
                    if((num &0x0F) == 0){
                        cntFirst++;
                    }
                    else if((num & 0x0f) == 1){
        ...
        if (elapsed >= 100) {
            std::cout<<"first "<<cntFirst<<" second "<<cntSecond<<std::endl;
            emit packetCountPair(PHOTONS_IN_PACKET*cntFirst,PHOTONS_IN_PACKET*cntSecond); // отправляем накопленное количество (может быть 0)
            cntFirst = 0;
            cntSecond = 0;
            lastTime = now;
        }
