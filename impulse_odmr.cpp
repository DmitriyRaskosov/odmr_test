...
#include <iostream>
#include <signal.h>
int setupSpincore(Impulse_config conf) {
    ...
    PyObject* pFunc = PyObject_GetAttrString(pModule, "build_impulses_for_impulse_odmr");
    if (!pFunc || !PyCallable_Check(pFunc)) {
        PyErr_Print();
        printf("ERROR: Function 'build_impulses_for_impulse_odmr' not found or not callable\n");
        return 1;
    }
    ...
    PyTuple_SetItem(pArgs, 9, PyLong_FromLong(conf.t8));
    PyTuple_SetItem(pArgs, 10, PyLong_FromLong(conf.N));
    PyTuple_SetItem(pArgs, 11, PyLong_FromLong(conf.delay_between_measurements));
...
int main(int argc, char **argv){
    ...
    t1.detach();
    auto c = ConfigParser("impulse_odmr.ini");
    auto conf = c.parse_impulse_config();

    Rigol_config_impulse rigol = c.parse_rigol_config_impulse();
    ...
    std::cout<<total_time_us<<std::endl;
    std::cout<<"N:"<<conf.N<<std::endl;
    ...
    total_time_ms += static_cast<unsigned long long>((static_cast<double>(conf.t8)/1000000.0 +10.0/1000.0) *points);
