...
lib_paths = [
    "libspinapi.so",                           # системный путь
    os.path.join(os.path.dirname(__file__), "libspinapi.so"),  # текущая директория
    "/usr/local/lib/libspinapi.so",           # /usr/local/lib
    "/usr/lib/libspinapi.so",                 # /usr/lib
]

spinapi = None
for path in lib_paths:
    try:
        spinapi = ctypes.CDLL(path)
        print(f"Successfully loaded spinapi from: {path}")
        break
    except OSError as e:
        continue

if spinapi is None:
    print("Failed to load spinapi library from any path.")
    print("Please ensure libspinapi.so is installed.")
